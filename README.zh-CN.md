...
| 随时修改创作内容 | 重新打开任意已发布角色或故事，修改声音、私人意图、阵容、Lore、Scene、Action、Agenda 与元数据。 |
| 不写代码也能创作 | 用自然语言描述角色或故事，检查草稿后再发布。 |
...
每个已发布 Story 都有可编辑的 `harness-tavern-story/v2` 源文件：
...
角色与故事发布后仍可通过完整的可视化工作台继续编辑。系统 ID、因果事件历史与已经发生的存档事实保持独立只读，因此修改作品本身不会篡改某次游玩中已经发生的事情。
...
0.13.0 是一个**本地优先、单 Owner Beta**。它已经可以用于真实的本地角色扮演、创作、迁移和继续开发，但不是经过独立安全审计的多租户托管服务。把它暴露到本机以外之前，请阅读[安全设计](docs/SECURITY.md)和[运维指南](docs/OPERATIONS.md)。
...
| 创建角色或故事 | [创作者指南](docs/CREATOR_GUIDE.md) |
