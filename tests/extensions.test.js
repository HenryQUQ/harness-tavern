...
await test('installed optional blueprints are inventoried without becoming core generation behavior', async t => {
  ...
  const created = app.library.add({ kind: 'story', content: { title: 'No generated clockwork', cast: [{ character: { name: 'No generated mechanist' } }] } })
  assert.deepEqual(created.item.tags, [])
  assert.equal(created.item.cast[0].character.personality, '')
})
